...
void SOLAR303Engine::noteOn(int midiNote, float velocity)
{
    ...
    else
    {
        ...
        vcaAttacking = true;   // VCA ramps up smoothly — no jump → no click
        envGate      = true;
    }
...
void SOLAR303Engine::allNotesOff()
{
    heldNotes.clear();
    envGate      = false;
    // NOTE: envVal and vcaVal are NOT zeroed instantly — they fade out
    // naturally through the 5ms release coefficients in processSample().
    // Instant zero = click/pop. Let the RC release handle it smoothly.
    noteAccented = false;
...
float SOLAR303Engine::processSample()
{
    ...
    if (pSquareWave)
    {
        ...
        osc -= polyBlep(phase, dt);          // falling edge at wrap — subtract to smooth
        float p2 = phase + (1.f - kSquareDuty);
        if (p2 >= 1.f) p2 -= 1.f;
        osc += polyBlep(p2, dt);             // rising edge at duty — add to smooth
    ...
    {
        phase -= 1.f;
        // 3. Soft reset — only on Saw (Square already handled by PolyBLEP)
        if (!pSquareWave)
            osc *= (1.f - kSoftReset);
    }
    ...
    // ══════════════════════════════════════════════════════════════════════════
    if (envGate && vcaAttacking)
    {
        // 0.15 ms attack — completely inaudible, just prevents VCA jump → click
        const float atkCoeff = 1.f - std::exp(-1.f / (0.00015f * srf));
        vcaVal += (1.f - vcaVal) * atkCoeff;
        if (vcaVal >= 0.9999f) { vcaVal = 1.f; vcaAttacking = false; }
    }
    else if (!envGate)
    {
        vcaVal      *= std::exp(-1.f / (0.005f * srf));   // 5 ms release
        vcaAttacking = false;
    }
