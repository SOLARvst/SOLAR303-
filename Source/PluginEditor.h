...
public:
    ...
    void mouseUp(const juce::MouseEvent& e) override
    {
        ...
        for (int oct = 0; oct <= 5; ++oct)
        {
            ...
            for (int n = 0; n < 12; ++n)
            {
                const int midi = (oct + 2) * 12 + n;   // C0=24 … C5=84 (Cubase: C3=MIDI60)
                if (midi > 84) break;
                sub.addItem(midi + 1,
...
// ── ML button — left click = learn, right/ctrl+click = clear ─────────────────
class MlButton : public juce::TextButton
{
public:
    ...
    void mouseDown(const juce::MouseEvent& e) override
    {
        ...
        const bool isCtrl  = e.mods.isLeftButtonDown() && e.mods.isCtrlDown();
        wasSpecialClick = isRight || isCtrl;
        if (wasSpecialClick) { if (onRightClick) onRightClick(); return; }
        juce::TextButton::mouseDown(e);
    }
    void mouseUp(const juce::MouseEvent& e) override
    {
        if (wasSpecialClick) { wasSpecialClick = false; return; }
        juce::TextButton::mouseUp(e);
    }
private:
    bool wasSpecialClick { false };
};
...
private:
    ...
    juce::TextButton resetSeqButton;
    ...
    MlButton mlUpButtons  [6];   // ↑ button per knob — right-click = clear
    MlButton mlDownButtons[6];   // ↓ button per knob — right-click = clear
    ...
    // ── Scale Lock (for RND operations) ──────────────────────────────────────
    juce::ComboBox   rootCombo;           // root note: C … B
    juce::ComboBox   scaleCombo;          // scale type
    juce::TextButton scaleLockButton { "LOCK" };  // toggle
    juce::Label      lRootCombo, lScaleCombo, lScaleLock;
    int rndRootNote { 0 };   // 0=C … 11=B
    int rndScaleIdx { 0 };   // index into scale table

    // ── Note Range (LOW NOTE / HIGH NOTE for RND) ─────────────────────────────
    juce::TextButton rndLowNoteButton;    // shows e.g. "C1"
    juce::TextButton rndHighNoteButton;   // shows e.g. "C4"
    juce::Label      lRndLowNote, lRndHighNote;
    int rndMinNote { 36 };   // C1 in Cubase (MIDI 36)
    int rndMaxNote { 72 };   // C4 in Cubase (MIDI 72)

    static juce::String midiNoteName (int midi);   // "C1", "F#3", etc.
    void showNoteRangePicker (bool isLow);          // popup for LOW / HIGH
    ...
    // ── Skin system ────────────────────────────────────────────────────────────
    SOLAR303Skin currentSkin { SOLAR303Skins::LightGray };
...
private:
    ...
    std::unique_ptr<ActivationScreen> activationScreen;

    // ── Distortion blink animation ────────────────────────────────────────────
    float distBlinkPhase = 0.f;   // 0–1, sine-driven slow pulse
